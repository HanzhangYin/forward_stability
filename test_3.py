...
NUM_RANDOM_SAMPLES = 10000  # e.g., 10000 random pairs instead of all pairs
...
def generate_stability_chart(n, num_samples=None, random_seed=None):
    ...
    total_possible_pairs = len(perms) ** 2
    
    # Determine if we're sampling or computing all pairs
    if num_samples is None or num_samples >= total_possible_pairs:
        # Compute all pairs
        use_sampling = False
        num_to_process = total_possible_pairs
    ...
    if use_sampling:
        ...
        pairs_processed = 0
        while pairs_processed < num_to_process:
            # Randomly select two permutations
            u = random.choice(perms)
            v = random.choice(perms)
            ...
            # Print progress every 1000 pairs (or adjust based on sample size)
            if pairs_processed % max(1, num_to_process // 10) == 0:
                print(f"Processed {pairs_processed} / {num_to_process} pairs...")
    else:
    ...
    # Add expectation to title
    expectation = compute_expectation(support_frequencies)
    title = f'Forward Stability Number Frequencies for S_{n} x S_{n}\n'
